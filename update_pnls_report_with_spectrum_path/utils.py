...
def extract_spectrum_data(
    ...
    """Récupère les données Spectrum depuis le fichier spécifié.

    Args:
        fp_spectrum: Chemin du fichier contenant les données Spectrum.

    Returns:
        DataFrame Polars avec les données consolidées
    """
    fp_spectrum = Path(workspace.files_path, fp_spectrum)

    if not fp_spectrum.exists():
        msg_error = (
            f"Le fichier de données Spectrum `{fp_spectrum.as_posix()}` n'existe pas, "
            "assurez-vous qu'il a été chargé dans le dossier `data/données spectrum/`."
        )
        current_run.log_error(msg_error)
        raise FileNotFoundError(msg_error)

    if fp_spectrum.suffix == ".parquet":
        df_spectrum = pl.read_parquet(fp_spectrum)

    elif fp_spectrum.suffix == ".csv":
        df_spectrum = pl.read_csv(fp_spectrum)

    elif fp_spectrum.suffix == ".xlsx":
        df_spectrum = pl.read_excel(fp_spectrum)

    else:
        msg_error = (
            f"Le fichier de données Spectrum `{fp_spectrum.as_posix()}` doit être au format "
            "`parquet`, `csv` ou `xlsx`. Le format actuel est `{fp_spectrum.suffix}`."
        )
        current_run.log_error(msg_error)
